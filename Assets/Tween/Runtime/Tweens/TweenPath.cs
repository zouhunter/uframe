...
namespace UFrame.Tween
{
    ...
    public class TweenPath : TweenBase
    {
        ...
        public float value
        {
            ...
            set
            {
                ...
                if (target != null)
                {
                    if (isWorld)
                    {
                        var nextPos = GetCRSPoint(mValue);
                        if (m_lookForward)
                        {
                            target.forward = nextPos - target.position;
                        }
                    ...
                    else
                    {
                        ...
                        if(m_lookForward)
                        {
                            target.forward = nextPos - target.localPosition;
                        }
        ...
        public TweenPath(Transform target,Vector3[] path,bool isWorld, float dureation = 1, float dely=0,bool lookForward = false)
        {
            this.target = target;
            this.path = path;
            ...
            this.delay = dely;
            this.m_lookForward = lookForward;
