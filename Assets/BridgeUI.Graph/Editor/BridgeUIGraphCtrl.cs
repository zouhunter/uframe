    ...
    public class BridgeUIGraphCtrl : NodeGraphController
    {
        ...
        private void CompleteResourceUIInfo(ResourceUIInfo rinfo)
        {
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(path))
                {
                    ...
                    rinfo.good = true;
                    rinfo.resourcePath = BridgeUI.Editors.BridgeEditorUtility.GetResourcesPath(path);
                }
                else
                {
                    rinfo.good = false;
        ...
        private void CompleteBundleUIInfo(BundleUIInfo binfo)
        {
            ...
            else
            {
                ...
                if (importer)
                {
                    binfo.bundleName = importer.assetBundleName = string.Format(UISetting.bundleNameFormat, obj.name.ToLower());
        ...
        {
            base.Build();
            var save = EditorUtility.DisplayDialog("生成脚本确认", "如需要更新面板名称脚本，请按确认！", "确认", "取消");
            if (save)
            {
        ...
        public override void BuildFromGraph(NodeGraphObj m_targetGraph)
        {
        ...
        public override void OnDragUpdated()
        {
            base.OnDragUpdated();
        ...
        {
            var pfbTrans = PrefabUtility.GetCorrespondingObjectFromSource(instenceObj);
            if (pfbTrans != null)
            {
                var prefab = PrefabUtility.GetOutermostPrefabInstanceRoot(pfbTrans as GameObject);
                if (prefab != null)
                {
        ...
        public override List<KeyValuePair<string, Node>> OnDragAccept(UnityEngine.Object[] objectReferences)
        {
