...
namespace UFrame.BehaviourTree
{
    public class TreeInfoDrawer
    {
        ...
        private void RebuildConditionList()
        {
            ...
            _conditionDrawer = new ConditionDrawer(_treeDrawer.target as BTree, _treeInfo, _treeDrawer);
        }
            ...
            using (var disableScope = new EditorGUI.DisabledGroupScope(!_treeInfo.enable))
            {
                ...
                using (var colorScope = new ColorScope(green, Color.green))
                {
                    DrawCreateNodeContent(nodeRect, _treeInfo.node, n =>
                    {
                        ...
                        _treeInfo.node = n;
                    }, _treeDrawer.target as BTree, _treeInfo.status, _treeInfo.tickCount, _treeDrawer.serializedObject, _treeDrawer.GetPropPath(_treeInfo.node));
                }
        ...
        public static void DrawCreateNodeContent<T>(Rect rect, T node, Action<T> onCreate, BTree tree, byte status, int tickCount, SerializedObject target = null, string propPath = null) where T : BaseNode
        {
            ...
            if (node != null)
            {
                ...
                using (var disableScope = new EditorGUI.DisabledScope(!EditorApplication.isPlaying && tree is BVariantTree))
                {
        ...
        private void Doublicat(object userData)
        {
            if (_parentTree != null)
            {
                ...
                // 创建新的TreeInfo
                var newInfo = TreeInfo.Create();

                // 复制当前节点的所有信息
                CopyPasteUtil.CopyTreeInfo(_treeInfo, newInfo, newInfo);
        ...
        {
            if (_treeDrawer.target is BVariantTree variantTree && variantTree && variantTree.baseTree)
            {
