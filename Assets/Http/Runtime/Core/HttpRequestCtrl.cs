...
namespace UFrame.Http
{
    ...
    public class HttpRequestCtrl
    {
        ...
        public float killTimeOutMuti = 3;
        ...
        protected virtual void RefreshRequest(HttpResponceInfo handle)
        {
            ...
            else
            {
                ...
                {
                    if(handle.request.downloadProgress < (Time.time - handle.startTime) / (handle.timeOut * killTimeOutMuti))
                    {
                        OnRequestTimeOut(handle);
                    }
                    else
                    {
                        Debug.LogFormat("continue download: {0} progress:{1} timeuse:{2} timeout:{3}", handle.requestId ,handle.request.downloadProgress,(Time.time - handle.startTime),handle.timeOut * killTimeOutMuti);
                    }
                }
            }
        }
        ...
        protected virtual void OnRequestFinish(HttpResponceInfo handle)
        {
            ...
            try
            {
                if (request != null)
                {
                    ...
                    {
                        finish = true;
                        OnRequestError(request.url, (int)request.responseCode,  request.error, true);
                    }
                }
            }
            catch (Exception x)
            {
                var errorInfo = $"download {request.url} error event callback exception:";
                Debug.LogError(errorInfo);
                Debug.LogException(x);
                OnRequestError(request.url, handle.responseCode, x.Message, true);
            }
        ...
        protected virtual void OnRequestTimeOut(HttpResponceInfo handle)
        {
            ...
            try
            {
                OnRequestError(url, (int)request.responseCode, " url is time out.", handle.retryCounter == 0);
            }
            catch (Exception x)
            {
            ...
            {
                Debug.Log($"timeout left:{handle.retryCounter},progress:{ handle.operation.progress } restart request:{url} timeuse:{Time.time - handle.startTime}");
                RetryRequest(handle);
            }
            else
            {
            ...
            if (m_requestInfoMap.TryGetValue(handle.requestId, out var requestInfos))
            {
                ...
                foreach (var requestInfo in requests)
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        OnRequestError(requestInfo.url, -1, e.Message, true);
                    }
                }
            }
        ...
        protected virtual void OnRequestError(string url, int code, string msg, bool finished)
        {
            ...
            {
                Debug.Log(errorText);
                onDownLandError?.Invoke(url, code, msg, finished);
            }
        }
