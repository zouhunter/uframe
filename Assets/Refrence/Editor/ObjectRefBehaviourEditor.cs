...
{
    [CustomEditor(typeof(ObjectRefBehaviour), true)]
    public class ObjectRefBehaviourEditor : Editor
    {
        protected SerializedProperty prop_description;
        protected ObjectRefBehaviour m_refView;
        ...
        protected virtual void OnEnable()
        {
            try
            {
                prop_description = serializedObject.FindProperty("m_description");
                m_refView = target as ObjectRefBehaviour;
                RebuildListView();
                InitSubTypes();
            }
        ...
        private void RebuildListView()
        {
            ...
            m_list.elementHeightCallback = DrawElementHeight;
        ...
        protected virtual void InitSubTypes()
        {
            if (m_subTypes == null)
            {
                ...
                var types = typeof(ObjectRefBehaviour).Assembly.GetTypes();
                for (int i = 0; i < types.Length; i++)
                {
                    if (types[i].IsSubclassOf(typeof(ObjectRefBehaviour)) && !types[i].IsAbstract)
                    {
        ...
        protected virtual void DrawElement(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            var flagRect = rect.Move(10, 2).ReSize(rect.width * 0.25f, rect.height * 0.9f);
            var targetRect = flagRect.MoveX(flagRect.width + 10).ReSizeW((rect.width - flagRect.width - 10) * 0.3f);
            var descRect = targetRect.MoveX(targetRect.width + 10).ReSizeW((rect.width - 80 - targetRect.width - 20));

            EditorGUI.LabelField(indexRect, (index + 1).ToString(), EditorStyles.miniButtonRight);
            var refItemProp = m_refView.objRefs[index];
            if (refItemProp != null)
            {
                refItemProp.name = EditorGUI.TextField(flagRect, refItemProp.name);
                var newObjectRefValue = EditorGUI.ObjectField(targetRect, refItemProp.target, typeof(Object), true);
        ...
        protected virtual void DrawHeadRect(Rect rect)
        {
            ...
            if (Event.current.type == EventType.ContextClick && rect.Contains(Event.current.mousePosition))
            {
                ...
                menu.AddItem(new GUIContent("拷贝"), false, (state) =>
                {
                    ...
                    var refView = target as ObjectRefBehaviour;
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(prop_description);
            serializedObject.ApplyModifiedProperties();
