...
namespace UFrame.HiveBundle
{
    internal class AssetBundleTree : TreeView
    {
        ...
        protected override void RowGUI(RowGUIArgs args)
        {
            ...
            Color old = GUI.color;
            var enableShow = bundleItem.enable;
            if (bundleItem.bundle is Models.BundleVariantFolderInfo variantfolderInfo)
            {
                GUI.color = Models.Model.k_LightGrey; //new Color(0.3f, 0.5f, 0.85f);
            }
            else if (bundleItem.bundle is Models.BundleFolderInfo folderInfo)
            {
                enableShow = folderInfo.CheckEnableDeepth();
            }
            if (!enableShow)
            {
            ...
            var indexRect = new Rect(right - size - 30, args.rowRect.yMin, 40, size);
            GUI.Label(indexRect, $"{args.row + 1})");
        }
        ...
        protected override void ContextClickedItem(int id)
        {
            ...
            if (selectedNodes.Count == 1)
            {
                if ((selectedNodes[0].bundle as Models.BundleFolderConcreteInfo) != null)
                {
                    ...
                    var dataInfo = selectedNodes[0].bundle as Models.BundleFolderConcreteInfo;
                    if (dataInfo != null)
                    {
                        var bundlePath = dataInfo.m_Name.bundleName;
                        menu.AddItem(new GUIContent("Split Folder Bundles"), false, SplitFolderBundles, bundlePath);
                        menu.AddItem(new GUIContent("Merge Folder Bundles"), false, MergeFolderBundles, bundlePath);
                    }
                }
                ...
                else
                {
                    ...
                    var dataInfo = selectedNodes[0].bundle as Models.BundleDataInfo;
                    if (dataInfo != null)
                    {
                        menu.AddItem(new GUIContent("Split Folder Bundles"), false, SplitFolderBundles, dataInfo.m_Name.bundleName);
                        menu.AddItem(new GUIContent("Group Bundle"), false, GroupBundles, dataInfo.m_Name.bundleName);
                        menu.AddItem(new GUIContent("Folder Bundles"), false, MutiBundles, dataInfo.m_Name.bundleName);
                    }
                }
                ...
                var nodeEnable = selectedNodes[0].enable;
                if (selectedNodes[0].bundle is Models.BundleFolderInfo folderInfo)
                {
                    nodeEnable = folderInfo.CheckEnableDeepth();
                }
                if (!nodeEnable)
                    menu.AddItem(new GUIContent("Mark " + selectedNodes[0].displayName + " enable"), false, EnableBundles, selectedNodes);
                else
                    menu.AddItem(new GUIContent("Mark " + selectedNodes[0].displayName + " disable"), false, DisableBundles, selectedNodes);
            }
            else if (selectedNodes.Count > 1)
            {
                ...
                menu.AddItem(new GUIContent("Mark " + selectedNodes.Count + " selected bundles enable"), false, EnableBundles, selectedNodes);
                menu.AddItem(new GUIContent("Mark " + selectedNodes.Count + " selected bundles disable"), false, DisableBundles, selectedNodes);
            }
            menu.ShowAsContext();
        }

        private void MergeFolderBundles(object userData)
        {
            var bundlePath = userData.ToString();
            Models.Model.DataSource.MergeFolderBundles(bundlePath);
            ForceReloadData(null);
        }

        private void SplitFolderBundles(object userData)
        {
            var path = userData.ToString();
            Models.Model.DataSource.SplitFolderBundles(path);
            ForceReloadData(null);
        }

        private void MutiBundles(object userData)
        {
            var path = userData.ToString();
            Models.Model.DataSource.SetBundleNameRule(path, NameRule.FileName);
            ForceReloadData(null);
        }

        private void GroupBundles(object userData)
        {
            var path = userData.ToString();
            Models.Model.DataSource.SetBundleNameRule(path, NameRule.Group);
            ForceReloadData(null);
        }
        ...
        void EnableBundles(object b)
        {
            var selectedNodes = b as List<Models.BundleTreeItem>;
            Models.Model.HandleBundleEnable(selectedNodes.Select(item => item.bundle), true);
            ReloadAndSelect(new List<int>());
        }
        void DisableBundles(object b)
        {
            var selectedNodes = b as List<Models.BundleTreeItem>;
            Models.Model.HandleBundleEnable(selectedNodes.Select(item => item.bundle), false);
            ReloadAndSelect(new List<int>());
        }
        void DeleteBundles(object b)
        {
            if (EditorUtility.DisplayDialog("Delete bundles", "Are you sure you want to delete the selected bundles?", "Delete", "Cancel") == false)
                return;
            var selectedNodes = b as List<Models.BundleTreeItem>;
