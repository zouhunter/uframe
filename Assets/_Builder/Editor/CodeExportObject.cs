...
namespace UFrame
{
    ...
    public class CodeExportObject : ScriptableObject
    {
        ...
        public List<ReplaceString> replaceStrings = new List<ReplaceString>();
        public List<string> replaceFileExts = new List<string>();
        [System.Serializable]
    ...
    public class CodeExportObjectEditor : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            sourceDir = $"{Application.dataPath}";
            moduleDirs = LoadModuleDirs();
            ...
            moduleList.onSelectCallback += OnSelectElement;
            obj.buildInfos.RemoveAll(x => System.Array.Find(moduleDirs, y => x.dirName == System.IO.Path.GetRelativePath(sourceDir, y)) == null);
        }

        private string[] LoadModuleDirs()
        {
            var dirs = System.IO.Directory.GetDirectories(sourceDir);
            var supportDirs = new List<string>();
            foreach (var dir in dirs)
            {
                var packageFile = System.IO.Path.Combine(dir, "package.json");
                if (System.IO.File.Exists(packageFile))
                    supportDirs.Add(dir);
            }
            return supportDirs.ToArray();
        }

        private void OnSelectElement(ReorderableList list)
        {
            var path = moduleDirs[list.index];
            var assetPath = System.IO.Path.GetRelativePath(System.Environment.CurrentDirectory, path);
        ...
        private void GetDrawElementCallback(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            var labelRect = new Rect(rect.x + 20, rect.y, rect.width * 0.2f, rect.height);
            ...
            var defineRect = new Rect(labelRect.x + labelRect.width, rect.y + 2, rect.width - labelRect.width - 150, rect.height - 4);
        ...
        private void OnDrawHeader(Rect rect)
        {
            ...
            var labelRect = new Rect(rect.x + 20, rect.y, rect.width * 0.2f, rect.height);
            var defineRect = new Rect(labelRect.x + labelRect.width, rect.y + 2, rect.width - labelRect.width - 150, rect.height - 4);
            EditorGUI.LabelField(defineRect, "Defines('|'):");
        ...
        private void CollectAllFiles(string dir, List<string> files, bool includeMeta)
        {
            ...
            foreach (var subDir in dirs)
            {
                CollectAllFiles(subDir, files, includeMeta);
            }
        }
        ...
        private string CollectCSharpFiles(string flags, string dir, List<string> files, bool editor)
        {
            string dllName = "";
            var defineFiles = Directory.GetFiles(dir, "*.asmdef", SearchOption.TopDirectoryOnly);
            foreach (var item in defineFiles)
            {
                ...
                if (editor == editorAssembly)
                {
                    ...
                    files.AddRange(topFiles);
                    dllName = System.IO.Path.GetFileNameWithoutExtension(item); ;
                }
            }
            var dirs = System.IO.Directory.GetDirectories(dir, "*", SearchOption.TopDirectoryOnly);
            foreach (var subDir in dirs)
        ...
        private void BuildDllAll(BuildTarget target)
        {
            ...
            var runtimeDllName = CollectCSharpFiles("*.cs", sourceFolder, runtimeFiles, false);
            var editorDllName = CollectCSharpFiles("*.cs", sourceFolder, editorFiles, true);
            ...
            {
                BuildAllDll(output, target, runtimeDllName, runtimeFiles.ToArray());
            }

            if (editorFiles.Count > 0)
            {
                BuildAllDll(output, target, editorDllName, editorFiles.ToArray());
            }
        }
        ...
        private void BuildCodeByPath(string path, bool top)
        {
            ...
            if (top)
            {
                ...
                foreach (var item in dirs)
                {
                    ...
                    {
                        CollectAllFiles(item, files, obj.includeMeta);
                    }
                }
            }
            else
            {
                CollectAllFiles(path, files, obj.includeMeta);
            }
            var counter = 0;
            ...
            foreach (var item in files)
            {
                ...
                isReplace = TryReplaceFileText(item, ref text);
        ...
        private bool TryReplaceFileText(string filePath, ref string fileText)
        {
            foreach (var item in obj.replaceFileExts)
            {
                if (!filePath.EndsWith(item))
                    continue;

                foreach (var replace in obj.replaceStrings)
                {
                    fileText = fileText.Replace(replace.oldStr, replace.newStr);
                }
                return true;
            }
            return false;
