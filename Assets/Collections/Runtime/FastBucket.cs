	...
	public class FastBucket<T> : FastEnumerable<T>
	{
		...
		public void InsertAt(T item, int index)
		{
			...
			else
			{
				...
				if (index < PeakCount)
				{
					int indexIndex = FastArray.BinarySearch(OpenSlots.innerArray, index);
					Shift(OpenSlots.innerArray, indexIndex, OpenSlots.innerArray.Length, -1);
				}
        ...
        public static void Shift(Array array, int min, int max, int shiftAmount)
        {
            ...
            FastArray.Copy(array, min, array, min + shiftAmount, max - min);
		...
		{
			int index = FastArray.IndexOf(innerArray, item);
