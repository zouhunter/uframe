...
namespace UFrame
{
    public class Singleton<Agent> : UFrame.AdapterBase
        where Agent : Singleton<Agent>, new()
    {
        ...
        protected void OnCreate()
        {
            if (Manage != null)
            {
                ...
                {
                    m_manager = null;
                    UnityEngine.Debug.LogError("manage not registed agent!" + this);
                }
            }
            else
            {
                UnityEngine.Debug.LogError("No GameManager! ,Failed RegistManager:" + this);
            }
        }
