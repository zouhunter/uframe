...
namespace UFrame
{
    ...
    {
        protected List<AdapterBase> m_agents = new List<AdapterBase>();
        ...
        protected virtual void InsetToOrderdList(int priority, IList<AdapterBase> list, AdapterBase obj)
        {
            int insertIndex = 0;
        ...
        internal void InsetToOrderdContextList(int priority, IList<RefreshContext> list, IInterval interal,AdapterBase agent, Action action)
        {
            int insertIndex = 0;
            for (; insertIndex < list.Count; insertIndex++)
            {
        ...
        protected virtual void OnRemoveAgent(AdapterBase manager)
        {
        ...
        internal void MarkDeathUpdates(List<RefreshContext> list, AdapterBase target)
        {
        ...
        /// 注册逻辑管理器
        /// </summary>
        public virtual bool RegistAgent(AdapterBase agent)
        {
            if (agent == null)
            {
                Debug.LogError("should not regist empty agent!");
                return false;
            }

            if(m_inUnRegistLoop)
            {
                Debug.LogError("should not regist agent in unregist loop!");
                return false;
            }
        ...
        //查找管理器
        public virtual T FindAgent<T>() where T:AdapterBase
        {
        ...
        //注销管理器
        public virtual bool RemoveAgent(AdapterBase agent)
        {
