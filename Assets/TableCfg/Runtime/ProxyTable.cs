...
namespace UFrame.TableCfg
{
    public abstract class ProxyTable<Proxy> : IProxyTable where Proxy : ProxyTable<Proxy>, new()
    {
        ...
        internal Dictionary<byte, ITableEncoder> m_tableEncoders = null;
        ...
        protected virtual void LoadTable<T>(string path, System.Action<string, bool> onLoad = null) where T : ITable, new()
        {
            ...
            m_textLoader.LoadStream(path, (style, streamContent) =>
            {
                ...
                var operation = decoder.Decode(table, streamContent, m_textLoader.Encoding,GetRowSerialize, CreateRow);
                operation.RegistFinish(() => {
        ...
        public void ExportTable(string filePath, byte style = 0)
        {
            if (!Finished)
            {
                Debug.LogError("table not load finished!");
                return;
            }
            ...
            ExportTable(style, filePath, Table);
        }

        protected AsyncTableOperation ExportTable(byte style, string filePath, ITable table)
        {
            string[] fields = table.Fields;
            if (m_tableEncoders == null)
                m_tableEncoders = new Dictionary<byte, ITableEncoder>();

            if (!m_tableEncoders.TryGetValue(style, out var encoder))
                encoder = m_tableEncoders[style] = new CsvTableEncoder();

            var fs = new System.IO.FileStream(filePath, System.IO.FileMode.OpenOrCreate, System.IO.FileAccess.Write, System.IO.FileShare.ReadWrite);
            var streamContent = new StreamContent(fs);
            var raws = new List<IRow>();
            table.GetRows(raws);

            if (fields == null)
            {
                var rawType = table.CreateRow().GetType();
                fields = rawType.GetFields().Select(x => x.Name).ToArray();
            }
            var operation = encoder.Encode(fields, raws, streamContent, GetRowSerialize, m_textLoader.Encoding);
            operation.RegistFinish(() =>
            {
    ...
    public abstract class ProxyTable<Proxy, K1, K2, Row> : ProxyTable<Proxy, K1, Row> where Proxy : ProxyTable<Proxy, K1, K2, Row>, new() where Row : IRow<K1, K2>, new()
    {
        ...
        public SortedDictionary<K2,Row> GetRowsByKey(K1 key)
        {
            var table = GetTable();
            if (table != null)
                return table.GetByKey(key);
    ...
    public abstract class ProxyTable<Proxy, K1, K2, K3, Row> : ProxyTable<Proxy, K1, K2, Row> where Proxy : ProxyTable<Proxy, K1, K2, K3, Row>, new() where Row : IRow<K1, K2, K3>, new()
    {
        ...
        public SortedDictionary<K3, Row> GetRowsByKey(K1 key,K2 key2)
        {
            var table = GetTable();
            if (table != null)
                return table.GetByKey(key,key2);
