...
{
    public class SubBindingViewCoderExecuter : ViewCoderExecuter
    {
        private string view_binding_partton = @"binding.(\w+).(\w+)=(\w+)";
        private string view_binding_func_partton = @"binding.(\w+).(\w+)\((\w+)\)";
        private string event_binding_partton = @"binding.(\w+).(\w+).AddListener\(On(\w+)\)";

        public SubBindingViewCoderExecuter(ViewCoder coder) : base(coder) { }
        ...
        private void OnBindingBlock(StringBuilder sb, int deepth)
        {
            ...
            Span(sb, deepth); sb.AppendLine(string.Format("if (target is {0})", refClassName));
            Span(sb, deepth); sb.AppendLine("{");

            var deepth1 = deepth + 1;

            Span(sb, deepth1); sb.AppendLine(string.Format("m_binding = target as {0};", refClassName));
