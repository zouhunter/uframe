...
{
    public class SubUIBindingViewCoderExecuter : ViewCoderExecuter
    {
        ...
        public SubUIBindingViewCoderExecuter(ViewCoder coder) : base(coder) { }
        ...
        {
            var parentName = string.IsNullOrEmpty(parentClassName) ? "BindingSubPanel" : parentClassName;
        ...
        private void ClassBlock(StringBuilder sb, int deepth)
        {
            ...
            if (componentItems != null)
            {
                ...
                Span(sb, deepth); sb.AppendLine("/// </summary>");
                Span(sb, deepth); sb.AppendLine("public override void Binding(BindingReference target)");
                Span(sb, deepth); sb.AppendLine("{");
                ...
                Span(sb, deepth); sb.AppendLine("/// </summary>");
                Span(sb, deepth); sb.AppendLine("public override void UnBinding()");
                Span(sb, deepth); sb.AppendLine("{");
        ...
        private void OnBindingBlock(StringBuilder sb, int deepth)
        {
            ...
            Span(sb, deepth); sb.AppendLine("if (target is UIBinding)");
            Span(sb, deepth); sb.AppendLine("{");

            var deepth1 = deepth + 1;

            Span(sb, deepth1); sb.AppendLine("FindElements(target as UIBinding);");
            ...
            for (int i = 0; i < componentItems.Length; i++)
            {
                ...
                for (int j = 0; j < eventBindings.Count; j++)
                {
                    ...
                    Span(sb, deepth1);
                    sb.AppendFormat("m_{0}.{1}.AddListener(On{2});", componentItem.name, eventItem.bindingTarget, eventItem.bindingSource);
                    sb.AppendLine();
                }
            }
