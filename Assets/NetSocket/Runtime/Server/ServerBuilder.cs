...
namespace UFrame.NetSocket
{
    public class ServerBuilder : BuilderBase<IServerBuilder, IServer, ServerBuilderOptions>, IServerBuilder
    {
        ...
        public override IServer Build()
        {
            ...
            //var serviceProvider = this.GetServiceCollection();

            CollectPackets();
            serviceCollection.GetService<IServerPacketProcessor>().SetUdpSender(serviceCollection.GetService<IUdpSocketSender>());
            return serviceCollection.GetService<IServer>();
        }
