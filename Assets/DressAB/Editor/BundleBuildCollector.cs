...
namespace UFrame.DressAB.Editors
{
    public class BundleBuildCollector
    {
        ...
        public void AutoSliceBundles(AddressDefineObject defObj)
        {
            ...
            Dictionary<string, AddressInfo> rootAssetPaths = new Dictionary<string, AddressInfo>();
            HashSet<string> singleBundleRefPaths = new HashSet<string>();
            foreach (var addressInfo in addressInfos)
            {
                ...
                if (Directory.Exists(path))
                {
                    ...
                    {
                        rootAssetPaths[item] = addressInfo;
                    }
                }
                else
                {
                    rootAssetPaths[path] = addressInfo;
                }
            }

            Dictionary<string, HashSet<string>> refAssetUseMap = new Dictionary<string, HashSet<string>>();
            foreach (var pair in rootAssetPaths)
            {
                ...
                foreach (var refPath in refPaths)
                {
                    if(info.single)
                    {
                        singleBundleRefPaths.Add(refPath);
                    }
            ...
            BuildAssetRefMap(rootAssetPaths.Keys, refAssetDirectUseMap);
            ...
            foreach (var refPair in refAssetUseMap)
            {
                ...
                if (singleBundleRefPaths.Contains(assetPath))
                {
                    Debug.Log("single bundle ignore:" + assetPath);
                    continue;
                }
                var parentList = refPair.Value;
                var guid = AssetDatabase.GUIDFromAssetPath(assetPath).ToString();
                if (parentList.Count >= defObj.useCoundMin && !rootAssetPaths.ContainsKey(assetPath))
                {
                    var importer = AssetImporter.GetAtPath(assetPath);
                    if (!string.IsNullOrEmpty(importer.assetBundleName))
                        continue;
