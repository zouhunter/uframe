...
{
    public class AssetBundleLoadCtrl:System.IDisposable
    {
        ...
        public void Dispose()
        {
            ...
            m_loadBundleCallBacks = null;
            m_bundleRefMap = null;
            m_asyncBundleOperationMap = null;
            m_catlogParser?.Dispose();
            m_catlogParser = null;
            disposed = true;
        }

        public AsyncCatlogOperation LoadCatlogAsync(string url,bool update=true)
        {
            ...
            m_updateChecker.StartCheckUpdate(url, update, operation);
            return operation;
        }
        ...
        private bool DoLoadAssetBundle(BundleItem bundleItem, Action<float> onProgress, Action<string, AssetBundle> onLoadFinish)
        {
            ...
            else
            {
                ...
                ParseBundleName(bundleName,out var shortName,out var fileHash);
        ...
        private static bool ParseBundleName(string bundleName,out string shortName,out string fileHash)
        {
            var splitIndex = bundleName.LastIndexOf("_");
            if(splitIndex > 0)
            {
                shortName = bundleName.Substring(0, splitIndex);
                fileHash = bundleName.Substring(splitIndex + 1);
                return true;
            }
            else
            {
        ...
        public static string HashMd5File(string filePath, int length)
        {
            ...
            {
                using (var mem = new System.IO.FileStream(filePath, System.IO.FileMode.Open, System.IO.FileAccess.ReadWrite, System.IO.FileShare.ReadWrite))
                {
        ...
        private void DoUnloadAssetBundle(BundleItem bundleItem, bool unloadAll)
        {
            if (disposed)
                return;
            if (m_loadedAssetBundles != null &&  m_loadedAssetBundles.TryGetValue(bundleItem.bundleName, out var bundle))
            {
            ...
            if (bundleItem != null && bundleItem.refs != null && bundleItem.refs.Length > 0)
            {
                foreach (var refBundle in bundleItem.refs)
                {
                    if (this.m_bundleRefMap.TryGetValue(refBundle.bundleName, out var parentBundles) && bundleItem!= null)
                    {
        ...
        private void OnLoadBundleFinish(BundleItem bundleItem, string filePath, AssetBundle assetBundle)
        {
            if (disposed)
            {
                ...
                Debug.LogError("assetbundle loadctrl disposed!");
                return;
            }
            m_deeploading.Remove(bundleItem);
            var bundleName = bundleItem.bundleName;
            if (assetBundle == null)
            {
                m_downloadingBundles.Remove(filePath);
        ...
        private void PreloadAssetBundle(BundleItem bundleItem, System.Action<string, object> onLoadBundle, HashSet<BundleItem> deepLoading)
        {
            ...
            ParseBundleName(bundleItem.bundleName, out var shortName, out var fileHash);
            ...
            {
                var remoteFile = $"{m_remoteUrl}/{fullName}";
                downloadFileFunc?.Invoke(remoteFile, localFile, (path,content)=> {
                    deepLoading.Remove(bundleItem);
                    onLoadBundle?.Invoke(path,content);
                }, null, null);
            }
            else
            {
