...
namespace UFrame.TableCfg
{
    public class TableScriptEditor
    {
        ...
        public static string CreateScript(string csvFile, out string scriptName, bool ignoreRef)
        {
            ...
            scriptName = System.IO.Path.GetFileNameWithoutExtension(csvFile);
        ...
        static string CreateProxyTableScript(string csvFile, out string scriptName)
        {
            ...
            var table = csvHelper.GetCacheData();
            csvHelper.ReadCSVToTableData(csvFile, System.Text.Encoding.UTF8, table);
            if (table != null)
            {
        ...
        static void CreateTableCtrlScript()
        {
            ...
            if (!string.IsNullOrEmpty(csvFolder))
            {
                ...
                if (files != null && files.Count > 0) if (files != null && files.Count > 0)
                    {
                        var savePath = EditorUtility.SaveFilePanel("保存表管理器脚本", LoadPreferFolder(export), "TableCtrl", "cs");
                        if (string.IsNullOrEmpty(savePath))
                            return;
                        var ctrlName = System.IO.Path.GetFileNameWithoutExtension(savePath);
                        var fileoutDir = System.IO.Path.GetDirectoryName(savePath);
                        var ctrlScript = MakeTableCtrlScript(ctrlName, csvFolder, files.ToArray());
                        if (!string.IsNullOrEmpty(ctrlScript))
                        {
                            SavePreferFolder(export, fileoutDir);
                            System.IO.File.WriteAllText(savePath, ctrlScript.Replace("\r\n", "\n"), System.Text.Encoding.UTF8);
                            AssetDatabase.Refresh();
                            OpenSelectFolder(fileoutDir);
                        }
                    }
            }
        }
        ...
        static void CreateTableScriptsFromCsvs()
        {
            ...
            {
                SavePreferFolder(import, csvFolder);
                var savePath = EditorUtility.SaveFilePanel("导入脚本路径", LoadPreferFolder(export), "TableCtrl", "cs");
                if (string.IsNullOrEmpty(savePath))
                    return;
                var ctrlName = System.IO.Path.GetFileNameWithoutExtension(savePath);
                var saveDir = System.IO.Path.GetDirectoryName(savePath);
                SavePreferFolder(export, saveDir);
                CreateTableScripts(csvFolder, ctrlName, saveDir);
                OpenSelectFolder(saveDir);
            }
        }
        ...
        public static void CreateTableScripts(string csvFolder, string ctrlName, string outDir)
        {
            ...
            if (deepFile)
                deepFile = EditorUtility.DisplayDialog("选择", "遍历表子目录？", "是", "否");
            var csvFiles = System.IO.Directory.GetFiles(csvFolder, "(*.csv|*.txt)", deepFile ? System.IO.SearchOption.AllDirectories : System.IO.SearchOption.TopDirectoryOnly);
            if (csvFiles == null || csvFiles.Length <= 0 || string.IsNullOrEmpty(outDir))
                return;
            ...
            var ctrlScript = MakeTableCtrlScript(ctrlName, csvFolder, csvFiles);
            if (!string.IsNullOrEmpty(ctrlScript))
            {
        ...
        private static string MakeScriptFromTable(string scriptName, TableData table)
        {
            try
            {
                ...
                WriteCfgScript("    ", sb, scriptName, classBaseType, keyProps, null, fields, types, descs);
                sb.AppendLine("}");
        ...
        private static string MakeTableCtrlScript(string scriptName, string csvRoot, string[] csvFiles)
        {
            ...
            for (int i = 0; i < cfgNum; i++)
            {
                ...
                csvHelper.ReadCSVToTableData(csvFiles[i], System.Text.Encoding.UTF8, csvTable);
                ...
                {
                    var match = System.Text.RegularExpressions.Regex.Match(csvTable.Descs()[0].ToString(), @"[\w.]+");
                    if (match.Success)
                    {
                        scriptTypes[i] = match.Value.Trim();
                    }
                }
        ...
        private static void WriteCfgScript(string span, System.Text.StringBuilder sb, string className, string classBaseType,
            List<System.Tuple<string, string, string>> keyProps, System.Type cfgType, List<string> fields, List<string> types, List<string> descs)
        {
            ...
            {
                sb.AppendLine();
                CreateLineReaderScript(sb, span, className, fields, types);
            }
        ...
        //csv line reader
        private static void CreateLineReaderScript(StringBuilder sb, string span, string className, List<string> fields, List<string> types)
        {
            var memberSpan = span + "    ";
