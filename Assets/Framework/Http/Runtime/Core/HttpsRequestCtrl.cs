...
namespace UFrame.Http
{
    public class HttpsRequestCtrl
    {
        ...
        protected virtual void OnRequestTimeOut(HttpsResponceInfo handle)
        {
            ...
            {
                Debug.Log($"timeout left:{handle.retryCounter},progress:{ handle.progress } restart request:{url} timeout:{handle.timeOut}");
                handle.timeOut *= 2;
                MarkRetryRequest(handle);
            }
            else
        ...
        {
            var errorText = $"https request error:{url} ,{msg}";
