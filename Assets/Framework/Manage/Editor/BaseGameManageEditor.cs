...
namespace UFrame.Editors
{
    ...
    public class BaseGameManageEditor : Editor
    {
        ...
        private void CreateInterfaceView(Type type, BaseGameManage manage, string filedName, ref ReorderableList viewList)
        {
            ...
            if (fileld != null)
            {
                var fileldValue = fileld.GetValue(manage) as System.Collections.Generic.List<Agent>;
                viewList = new ReorderableList(fileldValue, typeof(Agent), false, true, false, false);
                viewList.drawHeaderCallback = (rect) => { EditorGUI.LabelField(rect, string.Format("[Name,Priority]", typeof(Agent).FullName)); };
                viewList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
                {
